...
const seedAdmin = async () => {
    try {
        ...
        const existingAdmin = await userRepository.findOne({ where: { email: adminEmail }, select: ["id", "password"] });
        ...
        const existingStudent = await userRepository.findOne({ where: { email: studentEmail }, select: ["id", "password"] });

        if (existingStudent) {
            console.log("Test Student already exists. Resetting password...");
            existingStudent.password = await bcrypt.hash("Student123!", 10);
            await userRepository.save(existingStudent);
        } else {
        ...
        const existingSuperAdmin = await userRepository.findOne({ where: { email: superAdminEmail }, select: ["id", "password"] });

        if (existingSuperAdmin) {
            console.log("Super Admin user already exists. Resetting password...");
            existingSuperAdmin.password = await bcrypt.hash("SuperAdmin123!", 10);
