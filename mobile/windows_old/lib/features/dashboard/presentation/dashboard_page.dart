...
class DashboardPage extends StatelessWidget {
  ...
  Widget _buildQuickActions(BuildContext context) {
    final actions = [
      ...
      {'icon': Icons.assignment, 'label': 'Tasks', 'color': AppTheme.secondary},
      {'icon': Icons.auto_awesome, 'label': 'AI', 'color': AppTheme.accent},
      {'icon': Icons.forum, 'label': 'Discuss', 'color': Colors.blue},
      {'icon': Icons.folder_open, 'label': 'Resources', 'color': Colors.orange},
    ];
      ...
      itemBuilder: (context, index) {
        ...
        return InkWell(
          onTap: () {
            ...
            } else if (action['label'] == 'Tasks') {
              MainPage.of(context)?.setSelectedIndex(2);
            } else if (action['label'] == 'AI') {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const AIAssistantPage()),
              );
            } else if (action['label'] == 'Discuss') {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const DiscussionGroupListPage(),
                ),
              );
            } else if (action['label'] == 'Resources') {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const StudyMaterialListPage(),
                ),
              );
            }
          },
  ...
  Widget _buildAgenda(ScheduleState state) {
    ...
    final todaysSchedules = state.schedules
          ...
          builder: (context, state) {
            ...
            if (state.suggestions.isEmpty) {
              return _buildEmptyState(
                'No suggestions yet. Log your classes to see them!',
              );
            }
            return SizedBox(
              height: 150,
